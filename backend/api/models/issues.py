...
from .pullrequests import (
    PullRequestStatsBase,
    PullRequestsArrayCreate,
    PullRequestCreate,
)
...
class IssueBase(SQLModel):
    ...
    talking_to_myself: Optional[int] = (
        Field(..., description="No idea what that value is...?!?"),
    )
...
class Issue(IssueBase, table=True):
    ...
    first_reply_by: GithubUser = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Issue.first_reply_by_id]"}
    )
...
class IssueStatsAggregateCreate(SQLModel):
    ...
    updated: datetime
    stats: Union[
        StatsIssuePullRequestAggregateCreate, IssuesArrayCreate, PullRequestsArrayCreate
    ]  # daily stats aggregates plus two orphaned arrays weirdly intermixed there.
